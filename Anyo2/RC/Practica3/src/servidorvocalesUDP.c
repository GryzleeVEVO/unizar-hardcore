/*
    ...
    Descripción: Fuente del servidor del programa servidorvocalesTCP modificado convenientemente para
    realizar una conexión mediante el protocolo UDP

    Se han realizado las siguientes modificaciones:

    - Se han comentado las secciones del código relacionadas con las llamadas listen() y accept(), al
        ser innecesarias

    - Se han sustituido las llamadas send() y recv() por las llamadas sendto() y recvfrom(), propias del
        protocolo UDP, que se diferencian en tres puntos:

        * El campo del socket está ocupado por el socket del servidor

        * Se requiere un campo adicional donde se pasa un puntero de tipo sockaddr que almacenará, al
            recibir el datagrama, la información del cliente que lo envía, que luego utilizará como
            destino al enviar una respuesta

        * Se requiere un segundo campo de tipo socklen_t que, al recibir datos, almacene el tamaño del
            sockaddr devuelto, que luego requerirá al enviar una respuesta

    - Se ha eliminado la llamada close(conn), al ser inncecesaria
*/
