...
uint8_t C4_comprobar_columna(uint8_t c)
{
    if (C4_columna_valida(c))
    {
        ...
        while ((f <= NUM_FILAS) && (!celda_vacia(cuadricula[f][c])))
        {
            f++;
        }
                               ...
                               int8_t delta_fila, int8_t delta_columna)
{
    ...
    if (!C4_fila_valida(f) || !C4_columna_valida(c))
        return 0;

    // Acaba si la celda es vacía o de otro color
    if (celda_vacia(cuadricula[f][c]) || (celda_color(cuadricula[f][c]) != color))
        return 0;
...
uint8_t C4_hay_linea(uint8_t f, uint8_t c)
{
    ...
    for (i = 0; (i < 4) && (linea == 0); ++i)
    {
        ...
        long_linea +=
            C4_buscar_alineamiento(f - deltas_fila[i], c - deltas_columna[i],
                                   -deltas_fila[i], -deltas_columna[i]);
...
void C4_mostrar_tablero(uint8_t mostrar_marcada)
{
    for (uint8_t i = NUM_FILAS; i >= 1; i--)
    {
        ...
        {
            linea[k++] = '|';
            if (mostrar_marcada && fila == i && columna == j)
                linea[k++] = 'X';
            else if (celda_blanca(cuadricula[i][j]))
                linea[k++] = 'B';
            else if (celda_negra(cuadricula[i][j]))
                linea[k++] = 'N';
            else
                linea[k++] = ' ';
        }
...
void conecta4_detener(uint32_t razon)
{
    ...
    {
    case 0:
        serie_print("Victoria de las blancas");
        break;
    case 1:
        serie_print("Victoria de las negras");
        break;
    case 2:
        serie_print("Empate");
        break;
    case 3:
        serie_print("Detenido por el usuario");
        break;
    }
}
