...
public class CanibalesBoard {
	...
	// Por defecto
	public CanibalesBoard() { state = new int[] {3, 3, 0, 0, ORIGEN}; }
	...
	public void moveM(int n) {
		if (state[LB] == ORIGEN) 
			{ state[MO] -= n; state[MD] += n; state[LB] = DESTINO; } 
		else 
			{ state[MD] -= n; state[MO] += n; state[LB] = ORIGEN; }
	}
	// Mover n canibales
	public void moveC(int n) {
		if (state[LB] == ORIGEN) 
			{ state[CO] -= n; state[CD] += n; state[LB] = DESTINO; } 
		else
			{ state[CD] -= n; state[CO] += n; state[LB] = ORIGEN; }
	}
	
	// Mover un misionario y un canibal
	public void moveMC() {
		if (state[LB] == ORIGEN) { 
			state[MO] -= 1; state[MD] += 1; 
			state[CO] -= 1; state[CD] += 1;  
			state[LB] = DESTINO; 
		} 
		else { 
			state[MD] -= 1; state[MO] += 1; 
			state[CD] -= 1; state[CO] += 1; 
			state[LB] = ORIGEN; 
		}
	}
	...
	public boolean canMove(Action where) {		
		if 		(where.equals(M1M)) return canMoveM(1);
		else if (where.equals(M2M)) return canMoveM(2);
		else if (where.equals(M1C)) return canMoveC(1);
		else if (where.equals(M2C)) return canMoveC(2);
		else if (where.equals(M1M1C)) return canMoveMC();
		else return false;
	}
	...
	public String toString() {
		...
		switch (state[MO]) {
		case 0: MOstr += "     "; break;
		case 1: MOstr += "    M"; break;
		case 2: MOstr += "  M M"; break;
		case 3: MOstr += "M M M"; break;
		}
		
		switch (state[CO]) {
		case 0: COstr += "     "; break;
		case 1: COstr += "    C"; break;
		case 2: COstr += "  C C"; break;
		case 3: COstr += "C C C"; break;
		}
		
		switch (state[LB]) {
		case ORIGEN:  LBstr += " BOTE --RIO --      "; break;
		case DESTINO: LBstr += "      --RIO -- BOTE "; break;
		}
		
		switch (state[MD]) {
		case 0: MDstr += "     "; break;
		case 1: MDstr += "    M"; break;
		case 2: MDstr += "  M M"; break;
		case 3: MDstr += "M M M"; break;
		}
		
		switch (state[CD]) {
		case 0: CDstr += "     "; break;
		case 1: CDstr += "    C"; break;
		case 2: CDstr += "  C C"; break;
		case 3: CDstr += "C C C"; break;
		}
