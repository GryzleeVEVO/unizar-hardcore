...
test_encriptar_ascii() {
    ...
    ./MHcypher -e -f ./test/test1 -o cifrado $@
    ./MHcypher -d -f cifrado -o descifrado $@
    cmp ./test/test1 descifrado &>/dev/null </dev/null && bien || mal
    echo -ne "Test 2: 20KB de datos aleatorios "
    ./MHcypher -e -f ./test/test2 -o cifrado $@
    ./MHcypher -d -f cifrado -o descifrado $@
    cmp ./test/test2 descifrado &>/dev/null </dev/null && bien || mal
    echo -ne "Test 3: MHcypher.cc, utilizando codificación ASCII "
    ./MHcypher -e -f ./test/test3 -o cifrado $@
    ./MHcypher -d -f cifrado -o descifrado $@
    cmp ./test/test3 descifrado &>/dev/null </dev/null && bien || mal
}

test_encriptar_mayus() {
    echo -ne "Test 4: Ejemplo del enunciado (codif. mayúsculas) "
    ./MHcypher -mayus -e -f ./test/test4 -o cifrado $@
    ./MHcypher -mayus -d -f cifrado -o descifrado $@
    cmp ./test/test4 descifrado &>/dev/null </dev/null && bien || mal
    echo -ne "Test 5: 20KB de mayusculas aleatorias (codif. mayúsculas) "
    ./MHcypher -mayus -e -f ./test/test5 -o cifrado $@
    ./MHcypher -mayus -d -f cifrado -o descifrado $@
    cmp ./test/test5 descifrado &>/dev/null </dev/null && bien || mal
}
